  ...
  return (
    <Card className="rounded-lg border-[#DDE4D8] bg-white shadow-none">
      <CardHeader className="gap-3">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            <CardDescription className="text-xs font-medium uppercase tracking-[0.08em] text-[#66715F]">
              {metric.label}
            </CardDescription>
            <CardTitle className="text-2xl font-semibold leading-none tracking-normal text-[#101319] sm:text-3xl">
              {metric.value}
            </CardTitle>
          </div>
          <div
      ...
      <CardContent className="flex items-center justify-between gap-3">
        ...
        <Badge
          variant={isDanger ? "destructive" : isWarning ? "warning" : "success"}
          className="shrink-0"
        >
          {metric.tone === "good" ? (
